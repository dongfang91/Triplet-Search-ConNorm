...
class BatchHardSoftMarginTripletLoss(BatchHardTripletLoss):
    ...
    def forward(self, sentence_features: Iterable[Dict[str, Tensor]], labels: Tensor):
        ...
        return self.batch_hard_triplet_soft_margin_loss(labels, reps[:2])
    ...
    def batch_hard_triplet_soft_margin_loss(self, labels: Tensor, embeddings: Tensor) -> Tensor:
        ...
        embeddings_new = torch.cat((embeddings[0], embeddings[1]), 0)
        labels_new = torch.cat((labels, labels), 0)
        pairwise_dist = self.distance_metric(embeddings_new)


        # For each anchor, get the hardest positive
        # First, we need to get a mask for every valid positive (they should have same label)
        mask_anchor_positive = BatchHardTripletLoss.get_anchor_positive_triplet_mask(labels_new).float()
        ...
        mask_anchor_negative = BatchHardTripletLoss.get_anchor_negative_triplet_mask(labels_new).float()
